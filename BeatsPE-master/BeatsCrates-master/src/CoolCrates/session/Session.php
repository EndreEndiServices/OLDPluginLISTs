...
class Session {
    ...
    public function addCrateParticle(CrateBlock $crateBlock) {
        $particle = new CrateFloatingText($crateBlock->getCrate(), $crateBlock->add(0.5, 0.5, 0.5), Utils::translateColors("§7You have §8{$this->getCrateKey($crateBlock->getCrate()->getIdentifier())} §7keys!"));
    ...
    public function updateCrateParticles() {
        foreach($this->crateParticles as $particle) {
            $particle->setTitle(Utils::translateColors("§7You have §8{$this->getCrateKey($particle->getCrate()->getIdentifier())} §7keys!"));
