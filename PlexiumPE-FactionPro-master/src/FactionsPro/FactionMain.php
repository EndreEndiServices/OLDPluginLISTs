...
class FactionMain extends PluginBase implements Listener {
    ...
    public function getPlayersInFactionByRank($s, $faction, $rank) {
        ...
        while ($resultArr = $result->fetchArray(SQLITE3_ASSOC)) {
            ...
            if ($this->getServer()->getPlayerExact($row[$i]['player']) instanceof Player) {
                $team .= TextFormat::GRAY . $row[$i]['player'] . TextFormat::GREEN . " (Online) " . TextFormat::RESET;
            } else {
                $team .= TextFormat::GRAY . $row[$i]['player'] . TextFormat::RED . " (Offline) " . TextFormat::RESET;
            }
            $i = $i + 1;
        }
    ...
    public function getAllAllies($s, $faction) {
        ...
        while ($resultArr = $result->fetchArray(SQLITE3_ASSOC)) {
            ...
            $team .= TextFormat::RED . $row[$i]['faction2'] . TextFormat::GRAY . " | " . TextFormat::RESET;
            $i = $i + 1;
        }

        $s->sendMessage($this->formatMessage("- $faction Allies", true));
        $s->sendMessage($team);
    }
