...
class MainListener implements Listener {
    ...
    public function onLevelChange(EntityLevelChangeEvent $event) {
        ...
        if($event->getTarget()->getFolderName() != FCore::DEFAULT_LEVEL_NAME) {
            BossBarAPI::removeBossBar([$entity], $this->plugin->plugin->barEid);
        }
        else {
    ...
    public function onMove(PlayerMoveEvent $event) {
        $player = $event->getPlayer();
        if($player->getLevel()->getBlock($player)->getId() == 148)  {
            $this->plugin->plugin->parkourMgr->start($player);
        }
        elseif ($player->getLevel()->getBlock($player)->getId() == 147) {
            $this->plugin->plugin->parkourMgr->end($player);
        }
    }
    ...
    public function onDamage(EntityDamageEvent $event) {
        ...
        if($entity instanceof Player && $entity->getLevel()->getFolderName() == FCore::PARKOUR_LEVEL && $event->getCause() == EntityDamageEvent::CAUSE_VOID) {
            $this->plugin->plugin->parkourMgr->cancel($entity);
            $event->setCancelled(true);
            return;
        }
        if(!$entity instanceof Player && $entity instanceof Human) {
            ...
            if(isset($entity->namedtag->Minigame)) {
    ...
    public function onInteract(PlayerInteractEvent $event) {
        ...
        if($player->getLevel()->getTile($event->getBlock()) instanceof EnderChest) {
            $this->plugin->plugin->scheduleMgr->repeating["mystery"]->setPlayer($player);
            $event->setCancelled(true);
        }
