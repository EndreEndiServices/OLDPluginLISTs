...
class FineJoinEffects extends PluginBase implements Listener {
	...
	public function onEnable() {
		...
		$this->getLogger()->info(TextFormat::GREEN . "Я сдох :Р");
  }

  public function onJoin(PlayerJoinEvent $event) {
    if($event->getPlayer()->hasPermission("finejoineffects.player")) {
        $cg=$this->getConfig();
        $p=$event->getPlayer();
    ...
    if($jnmsg == true){
        $p->sendMessage("§4➥ §eTroll§cCraft §a $msg");
  }
 }
    if($event->getPlayer()->hasPermission("finejoineffects.vip")) {
        $cg=$this->getConfig();
        $p=$event->getPlayer();
    ...
    if($jnmsgv == true){
        $p->sendMessage("§4➥ §eTroll§cCraft §a $msgv");
  }
 }
}
	public function onDisable() {
		$this->getLogger ()->info (TextFormat::RED . "Я жив :Р" );
          $this->saveDefaultConfig();
	}
}