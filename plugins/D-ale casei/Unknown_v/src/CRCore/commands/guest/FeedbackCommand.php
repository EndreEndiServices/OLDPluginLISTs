...
class FeedbackCommand extends BaseCommand{
    ...
    public function __construct(Loader $plugin){
        parent::__construct($plugin, "feedback", "Dane un feedback", "/feedback", ["fb"]);
    }
    ...
    public function sendFeedbackForm(Person $person) : void{
        ...
        $form = $api->createCustomForm(function (Person $player, array $data){
            ...
            fwrite($f, $data[0] . "\n");
            fclose($f);
            $player->sendMessage(self::FEEDBACK_PREFIX . TextFormat::GREEN . "Multumim pentru feedback! scripterii nostri se vor uita mai tarziu.");
            return null;
        });
        $form->setTitle(TextFormat::BLUE . "Feedback");
        $form->addInput("Scrie feedback-ul/sugestia/bug-ul aici!\n", "Vault-urile nu merg..");
