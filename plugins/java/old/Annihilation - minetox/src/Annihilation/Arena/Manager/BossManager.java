    ...
    public void spawnBoss(int boss){
        if(plugin.phase >= 3){
            plugin.messageAllPlayers(TextFormat.GRAY+"================[ "+TextFormat.DARK_AQUA+"Boss"+TextFormat.GRAY+" ]================\n"
                    + getBossName(boss)+TextFormat.GRAY+" has respawned! Go slay the beast!\n"
                    + TextFormat.GRAY+"=======================================");
        }
        ...
        Entity golem = Entity.createEntity(EntityHuman.NETWORK_ID, chunk, getNbt());

        golem.setMaxHealth(200);
        golem.setHealth(200);

        String name = plugin.data.get("boss" + boss + "name") + "   " + TextFormat.GREEN + golem.getHealth() + " HP";
