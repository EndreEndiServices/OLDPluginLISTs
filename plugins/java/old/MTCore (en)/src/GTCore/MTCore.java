...
public class MTCore extends PluginBase implements Listener {
    ...
    public void onLoad() {
        instance = this;
        serverFullMessage = TextFormat.RED + "Sorry, but this server is full. " + TextFormat.YELLOW + "You can buy a " + TextFormat.AQUA + "VIP rank at " + TextFormat.GREEN + "gameteam.cz/vip-a-kredity/";
    ...
    public void onEnable() {
        ...
        MessageTask.messages.add(TextFormat.AQUA + "For more information visit our website " + TextFormat.GREEN + "www.gameteam.cz");

        this.anni = (Annihilation) getServer().getPluginManager().getPlugin("Annihilation");
    ...
    public void enableItemRemoving() {
        this.getServer().getScheduler().scheduleDelayedRepeatingTask(new Runnable() {
            ...
            public void run() {
                ...
                getLogger().info(MTCore.getPrefix() + TextFormat.GREEN + "Removed " + count + " items.");
            }
        }, 303 * 20, 304 * 20);
    }
    ...
    public void onLogin(PlayerLoginEvent e) {
        ...
        if (getServer().getOnlinePlayers().size() >= 80 && !p.hasPermission("gameteam.vip")) {
            e.setCancelled();
            e.setKickMessage(TextFormat.RED + "Server is full");
            return;
        }
        ...
        if (sender instanceof Player) {
            switch (cmd.getName().toLowerCase()) {
                ...
                case "register":
                    ...
                    } else if (args[1].length() < 4 || args[1].length() > 16) {
                        sender.sendMessage(MTCore.getPrefix() + TextFormat.RED + "Password length must be between 4 and 16 characters");
                    } else {
    ...
    public void onChat(PlayerChatEvent e) {
        ...
        if (!data.isLoggedIn()) {
            ...
            p.sendMessage("§cPlease wait " + (data.getNextChat() - time) + " seconds");
        } else {
            if (!p.isOp()) {
    ...
    public void onPlayerInteract(PlayerInteractEvent e) {
        ...
        if ((action == PlayerInteractEvent.LEFT_CLICK_BLOCK || action == PlayerInteractEvent.RIGHT_CLICK_BLOCK) && target.distance(player) > 5) {
            e.setCancelled();
            return;
        }
        ...
        if (!data.isLoggedIn()) {
            p.sendMessage(getPrefix() + TextFormat.RED + "You are not logged in");
            e.setCancelled();
        }/* else {
    ...
    public void onPlayerDropItem(PlayerDropItemEvent event) {
        if (!this.isAuthed(event.getPlayer())) {
            event.getPlayer().sendMessage(getPrefix() + TextFormat.RED + "You are not logged in");
            event.setCancelled();
        }
    ...
    public void onMove(PlayerMoveEvent e) {
        ...
        if (p.onGround || p.riding != null || data.isOnGround() || !NoCheatTask.isInAir(p, data)) {
            data.setLastOnGround(System.currentTimeMillis());
            ...
            if (to.y >= from.y && to.y > 0) {
                Location loc = data.getLastJumpPos();

                double actualDistance = loc.distance(to);
                ...
                if ((slabDistance > 0.3 || slabDistanceY > 0.6 || time - data.getLastSlabTime() > 500) && groundDistance > 0.6 && data.getLastLiquid().distance(to) > 0.6) {
                    //System.out.println("Y diff: "+(to.y - pos.y));
                    if (time - data.getLastHit() > 3000 && (time - data.getLastJump() > 3000 || (actualDistance > 3.2 && to.y >= loc.y) || to.y - loc.y > 1.3 || inAirTime > 3000)) {
    ...
    public void onDataPacketReceive(DataPacketReceiveEvent e) {
        ...
        if (e.getPacket().pid() == ProtocolInfo.PLAYER_ACTION_PACKET) {
            if (((PlayerActionPacket) e.getPacket()).action == PlayerActionPacket.ACTION_JUMP && e.getPlayer().onGround) {
                PlayerData data = getPlayerData(e.getPlayer());
