...
public class Arena extends ArenaManager implements Listener {
    ...
    public void onDeath(PlayerDeathEvent e) {
        ...
        if (this.phase >= 1) {
            new NormalQuery(Annihilation.getInstance(), Stat.DEATHS, new String[]{p.getName().toLowerCase()});
    ...
    public void onInteract(PlayerInteractEvent e) {
        ...
        if (b.getId() == Item.WALL_SIGN || b.getId() == Item.SIGN_POST) {
            ...
            if (sign != null && TextFormat.clean(sign.getText()[0].toLowerCase()).contains("[shop]")) {
                ...
                if (line2.contains("weapon")) {
                    ...
                    if (this.phase < 4) {
                        p.sendMessage(TextFormat.RED + "You can not use brewing until phase IV");
                        e.setCancelled();
                        return;
                    }
        ...
        if (b.getId() == Item.CHEST) {
            if (isEnderChest(b)) {
                e.setCancelled();
                p.sendMessage(Annihilation.getPrefix() + TextFormat.GRAY + "This is your team's Ender Chest. Any items you store here are safe from all other players.");
    ...
    public void startGame(boolean force) {
        ...
        if (this.getAllPlayers().size() < 1 && !force) {
            this.messageAllPlayers(Annihilation.getPrefix() + TextFormat.RED + "required 16 players to start");
            this.starting = false;
            this.task.time1 = 120;
            return;
        }
    ...
    public void onNexusBreak(Player player, Team damagedTeam) {
        ...
        new NormalQuery(Annihilation.getInstance(), Stat.NEXUSDMG, new String[]{player.getName().toLowerCase()});
    ...
    public void joinTeam(Player p, int team2, boolean forceJoin) {
        ...
        if (pTeam != null && pTeam.getId() == team.getId()) {
            p.sendMessage(Annihilation.getPrefix() + TextFormat.GRAY + "You are already in " + team.getColor() + team.getName() + " team");
            return;
        }

        if (!this.isTeamFree(team) && !p.hasPermission("gameteam.vip")) {
            p.sendMessage(Annihilation.getPrefix() + TextFormat.GRAY + "This team is full");
            return;
        }
    ...
    public void onNexusDestroy(Player p, Team damagedTeam) {
        ...
        new NormalQuery(Annihilation.getInstance(), Stat.NEXUSES, new String[]{p.getName().toLowerCase()});
        ...
        String msg = TextFormat.GRAY + "===========[ " + damagedTeam.getColor() + "Nexus Destroyed" + TextFormat.GRAY + " ]===========\n"
        ...
        new NormalQuery(Annihilation.getInstance(), Stat.LOSSES, damagedTeam.getPlayers().keySet().toArray(new String[damagedTeam.getPlayers().size()]));
    ...
    public void onTransaction(InventoryTransactionEvent e) {
        ...
        if (shop != null) {
            ...
            p.sendMessage(TextFormat.GRAY + "Purchased " + TextFormat.YELLOW + item2.getName());
    ...
    public void checkNexuses() {
        ...
        if (alive.size() == 1) {
            ...
            this.ending = true;
            messageAllPlayers(TextFormat.GRAY + "================[ " + winnerteam.getColor() + "End Game" + TextFormat.GRAY + " ]================\n"
                    + TextFormat.GRAY + "Team " + winnerteam.getColor() + winnerteam.getName() + TextFormat.GRAY + " Wins Annihilation! Restarting game...\n"
                    + TextFormat.GRAY + new String(new char[42 - 1]).replace("\0", "="));
            this.winnerteam.message(TextFormat.BOLD + TextFormat.GOLD + "Recieved 40 coins for a win!");

            new NormalQuery(Annihilation.getInstance(), Stat.WINS, this.winnerteam.getPlayers().keySet().toArray(new String[this.winnerteam.getPlayers().size()]));
        }
    }
    ...
    public void onEat(PlayerItemConsumeEvent e){
        System.out.println(e.getItem().getName()+":    "+e.getItem().getId()+":"+e.getItem().getDamage());
    }
