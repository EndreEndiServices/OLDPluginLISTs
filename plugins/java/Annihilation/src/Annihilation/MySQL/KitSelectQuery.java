        ...
        if (plugin != null && plugin.isEnabled()) {
            ...
            if (arena != null) {
                if (!purchased && !p.isOp() && !p.hasPermission("gameteam.vip")) {
                    p.sendMessage(Annihilation.getPrefix() + TextFormat.RED + "Tento kit nemas zakoupeny");
                } else {
                    //$arena->kitManager->onKitChange($p, $this->kit);
                    arena.getPlayerData(p).setNewKit(kit);
                    p.sendMessage(Annihilation.getPrefix() + TextFormat.GREEN + "Vybral sis kit " + TextFormat.BLUE + kit.getName());
                }
            }
        }
    }
