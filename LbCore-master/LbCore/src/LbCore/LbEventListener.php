...
/**
 * Listen common plugin events like player Creation, Death, Chat etc.
 * Game plugins can extend this class if they need to use its logic
 */
class LbEventListener implements Listener {
	...
	public function onPlayerMove(PlayerMoveEvent $event) {
		$player = $event->getPlayer();
		$player->setLastMove(LbCore::getInstance()->getServer()->getTick());
	}

	public function switchItem(PlayerItemHeldEvent $event) {
		$player = $event->getPlayer();
		$itemId = $event->getItem()->getId();
		if($player->getState() == $player::IN_LOBBY){
			if ($itemId === Item::BUCKET) {
				if ($player->isVip()) {
					if($player->getParticleEffectId() != Item::BUCKET){
						$player->setParticleEffectId(Item::BUCKET);
						LbCore::getInstance()->particleManager->setPlayerParticleEffect($player, ParticleManager::$lava);
						$player->sendLocalizedPopup("PARTICLES", [TextFormat::RED . "Lava"]);
					}elseif($player->getParticleEffectId() == Item::BUCKET){
						$player->setParticleEffectId(0);
						$player->sendLocalizedPopup("PARTICLES_OFF");
					}
				} else {
					$player->sendLocalizedPopup("PARTICLE_FOR_VIP");
				}
                $player->getInventory()->setHeldItemIndex(0);
				$event->setCancelled(true);
			} else if ($itemId === Item::REDSTONE) {
				if ($player->isVip()) {
					if($player->getParticleEffectId() != Item::REDSTONE){
						$player->setParticleEffectId(Item::REDSTONE);
						LbCore::getInstance()->particleManager->setPlayerParticleEffect($player, ParticleManager::$redstone);
						$player->sendLocalizedPopup("PARTICLES", [TextFormat::DARK_RED . "Redstone"]);
					}elseif($player->getParticleEffectId() == Item::REDSTONE){
						$player->setParticleEffectId(0);
						$player->sendLocalizedPopup("PARTICLES_OFF");
					}
				} else {
				...
				if ($player->isVip()) {
					if($player->getParticleEffectId() != 120){
						$player->setParticleEffectId(120);
						LbCore::getInstance()->particleManager->setPlayerParticleEffect($player, ParticleManager::$portal);
						$player->sendLocalizedPopup("PARTICLES", [TextFormat::LIGHT_PURPLE . "Portal"]);
					}elseif($player->getParticleEffectId() == 120){
						$player->setParticleEffectId(0);
						$player->sendLocalizedPopup("PARTICLES_OFF");
					}
				} else {
				...
				if ($player->isVip()) {
					if($player->getParticleEffectId() != 378){
						$player->setParticleEffectId(378);
						LbCore::getInstance()->particleManager->setPlayerParticleEffect($player, ParticleManager::$rainbow);
						$player->sendLocalizedPopup("PARTICLES", [TextFormat::RED . "R" . TextFormat::GOLD . "a" . TextFormat::YELLOW . "i" . TextFormat::GREEN . "n" . TextFormat::BLUE . "b" . TextFormat::DARK_PURPLE . "o" . TextFormat::LIGHT_PURPLE . "w"]);
					}elseif($player->getParticleEffectId() == 378){
						$player->setParticleEffectId(0);
						$player->sendLocalizedPopup("PARTICLES_OFF");
					}
				} else {
    ...
    public function onPlayerDropItem(PlayerDropItemEvent $event){
        ...
        $item = $event->getItem();
        if(($player->getState() == $player::IN_LOBBY) &&
		   ($item->getId() == Item::BUCKET ||
            $item->getId() == Item::REDSTONE ||
            $item->getId() == 120 ||
            $item->getId() == 378)){
            $event->setCancelled();
        }
    ...
    public function onBlockPlace(BlockPlaceEvent $event){
        ...
        if($player->isInDeathmatch()){
            $event->setCancelled(true);
            $player->sendLocalizedPopup("CANT_PLACE_BLOCK");
        }
    }
...
//				}
		...
		/* Change Password */
		if ($player->isNeedNewPass() && $player->getChatMode() !== LbPlayer::CHAT_MODE_PWD_CHANGE) {
			$player->changeChatModeToPassChange(true);
			$player->sendLocalizedMessage('CHANGEPW_CHANGE', array($player->getName()));
			return;
		}
		...
		/* Chat filter */
		$lbcore = LbCore::getInstance();
		if (!is_null($lbcore) && $lbcore->filter->check($player, $message)) {
			/*
	...
	/*
	 * Kicks for unallowed players 
	 * (already authenticated, too may players from same ip)
	 * 
	...
	public function onPlayerPrePreprocess(PlayerCommandPreprocessEvent $event) {
		...
		if ($player instanceof LbPlayer) {
			if (strtolower(substr($msg, 0, strlen('/give'))) == '/give') {
				$msg = str_replace('/give', '/lbgive', $msg);
	...
	/**
	 * remove ignored players from recipients list
	 * 
	 * @param PlayerChatEvent $event
	 */
	...
	/**
	 * Collect players from the same ip
	 * 
	 * @param $ip
	 * @return array
	 */
