...
class VoteShop{
    ...
    public function Start($sender){
        ...
        $this->Vote = new Config($this->plugin->getDataFolder() . "Shop.yml", Config::YAML);

        $money = $this->plugin->ecoapi->myMoney($sender);
        $hasmoneycfg = $this->Vote->getNested("Vote.cost");
        ...
        if($money >= $hasmoneycfg){
            ...
            $voteitem = Item::get(131,2,1);
            $voteitem->addEnchantment(new EnchantmentInstance($e, (int) -0));
            $voteitem->setCustomName("§cVote");
            $sender->getInventory()->addItem($voteitem);
