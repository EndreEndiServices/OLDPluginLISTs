...
class Common{
    ...
    public function Start($sender){
        ...
        if($inv->contains(Item::get(131,1,1))){
            ...
            $name = $sender->getName();
            $Common = new Config($this->plugin->getDataFolder() . "Common.yml");
            $items = $Common->getNested("Common.items");
            ...
            $this->plugin->getServer()->broadcastMessage("§b$name §ajust opened §aCommon §aCrate!");
            $result = mt_rand(1,1);
                 switch($result){
        case 1:
        $sender->getInventory()->addItem(Item::get($values[0], $values[1], $values[2])->setCustomName($values[3]));
            ...
            $sender->sendMessage("§fYou don't have §aCommon §fKey.");
        }
    }
}
