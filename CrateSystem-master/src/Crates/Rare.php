...
class Rare{
    ...
    public function Start($sender){
        ...
        if($inv->contains(Item::get(131,3,1))){
            ...
            $name = $sender->getName();
            $Rare = new Config($this->plugin->getDataFolder() . "Rare.yml");
            $items = $Rare->getNested("Rare.items");
            ...
            $this->plugin->getServer()->broadcastMessage("§b$name §ajust opened §6Rare §aCrate!");
            $result = mt_rand(1,1);
                 switch($result){
        case 1:
        $sender->getInventory()->addItem(Item::get($values[0], $values[1], $values[2])->setCustomName($values[3]));
            ...
            $sender->sendMessage("§fYou don't have §6Rare §fKey.");
        }
    }
}
