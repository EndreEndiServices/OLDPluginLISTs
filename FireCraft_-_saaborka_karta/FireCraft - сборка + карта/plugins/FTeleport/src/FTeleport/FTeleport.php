        ...
        switch($cmd->getName())
		{
            case 'tpa':
                if ($sender instanceof Player)
                {
                    ...
                    {
                        if(trim(strtolower($sender->getName())) == trim(strtolower($args[0]))){$sender->sendMessage(F::RED."Ты не можешь телепортироваться к себе.");return true;}
                        $this->tp_sender = $sender->getName();
                        $this->tp_reciver = $args[0];
                        if($this->getServer()->getPlayer($args[0]) instanceof Player)
                        {
							$sender->sendMessage(F::YELLOW."Ты отправил запрос на телепортацию!");
							$this->getServer()->getPlayer($args[0])->sendMessage(F::RED."");
                            $this->getServer()->getPlayer($args[0])->sendMessage(F::YELLOW."Игрок ".F::AQUA.$this->tp_sender.F::YELLOW." отправил(а) запрос на телепортацию к тебе");
                            $this->getServer()->getPlayer($args[0])->sendMessage(F::YELLOW."Напиши: ".F::GREEN."/tpaccept".F::YELLOW." или ".F::GREEN."/tpc".F::YELLOW.", чтобы принять запрос");
                        ...
                        {
                            $sender->sendMessage(F::RED."Игрок не онлайн или ты ввел(а) неверный ник.");
                            return true;
                        }
                    }
                    else
                    {
                        $sender->sendMessage(F::RED."Используй: ".F::GREEN."/tpa (ник)");
                        return false;
                    }
                }
                else
			...
			case 'tpc':
                if ($sender instanceof Player)
                {
                    ...
                    if(count($sql) > 0)
                    {
                      ...
                      switch($sql['type'])
                      {
                          case 'tpa':
                              ...
                              {
								  $sender->sendMessage(F::RED."");
								  $sender->sendMessage(F::YELLOW."Ты принял запрос на телепортацию, от игрока ".F::GREEN.$this->getServer()->getPlayer($sql['player_from'])->getName());
								  $this->getServer()->getPlayer($sql['player_from'])->sendMessage(F::RED.""); 
								  $this->getServer()->getPlayer($sql['player_from'])->sendMessage(F::GREEN."Игрок принял твой запрос. ".F::YELLOW."Телепортация..");
                    ...
                    {
                        $sender->sendMessage(TextFormat::RED."§eУ вас нет активных запросов");
