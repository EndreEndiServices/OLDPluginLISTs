...
class DrTrayaurus extends PluginBase implements Listener, CommandExecutor{
        ...
        if(isset($args[1])){
                ...
                if(($p = $this->getServer()->getPlayer($args[1])) instanceof Player){
                    if($this->isDisguised($p->getID())){
                        ...
                        if(is_numeric($args[0])) {
                            ...
                            $this->e[$p->getID()] = $s;
                            $sender->sendMessage("DrTrayaurus Disguise activated for" . $p->getName());
            ...
            if($sender instanceof Player){
                if($this->isDisguised($sender->getID())){
                    ...
                    $sender->sendMessage("DrTrayaurus Disguise closed.");
                    return true;
                }
                else{
                    if(isset($args[0])){
                        if(is_numeric($args[0])) {
                            ...
                            $sender->sendMessage("DrTrayaurus Disguise activated.");
                        }
                        elseif(($mob = $this->getMobStore()->getMobId($args[0])) !== false){
                            ...
                            $sender->sendMessage("Thats not DrTrayaurus Name!");
                        }
                        return true;
                    }
                    else{
                        $sender->sendMessage("You need to specify the right DrTrayaurus");
                        return true;
                    }
                }
            }
    ...
    public function onPacketSend(DataPacketSendEvent $event){
        if(isset($event->getPacket()->eid)){
            if($this->isDisguised($event->getPacket()->eid) && !$event->getPlayer()->hasPermission(drtrayaurus.exempt)){
