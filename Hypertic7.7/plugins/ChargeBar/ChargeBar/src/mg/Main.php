...
class Main extends PluginBase implements Listener{
   ...
   $event->getPlayer()->setHealth(40);       
    ...
    $this->rew2[$event->getPlayer()->getName()] = $task2;
            $this->getServer()->getScheduler()->scheduleRepeatingTask($task2, 12);
        }
        if(array_search($event->getPlayer()->getName(), $this->rew3)){}else{
    $this->rew3[$event->getPlayer()->getName()] = $task3;
            $this->getServer()->getScheduler()->scheduleRepeatingTask($task3, 10);
        }
        }
    ...
    if($event->getCause() == 9){
        $p = $event->getEntity();
        if($p instanceof Player){
            if($p->getInventory()->contains(new Item(46, 0, 1)) == TRUE){
             $p->sendPopup(color::GREEN."Protected From Explosion!");
                $event->setCancelled();
            }
        }
    }
