...
use PlayNoteBlockSong1\task\LoadSongAsyncTask;
use PlayNoteBlockSong1\task\PlaySongTask;
use PlayNoteBlockSong1\task\Task;
...
class PlayNoteBlockSong extends PluginBase{
	...
	public function onCommand(CommandSender $sender, Command $cmd, $label, array $sub){
		...
		switch(strtolower($sub[0])){
 			case "play":
 			case "p":
				if(!$sender->hasPermission("playnoteblocksong1.cmd1.play1")){
			...
			case "stop":
			case "s":
				if(!$sender->hasPermission("playnoteblocksong1.cmd1.stop1")){
					$r = new Translation(Color::RED . "%commands.generic.permission");
				}elseif(!$this->play){
			...
			case "next":
			case "n":
				if(!$sender->hasPermission("playnoteblocksong1.cmd1.next1")){
					$r = new Translation(Color::RED . "%commands.generic.permission");
				}elseif(count($this->songs) <= 0){
					$r = Color::RED . "[PlayNBS] " . ($ik ? "당신은 음악이 하나도 없습니다." : "You don't have any song");
				}else{
			...
			case "prev":
			case "pr":
				if(!$sender->hasPermission("playnoteblocksong1.cmd1.prev1")){
					$r = new Translation(Color::RED . "%commands.generic.permission");
				}elseif(count($this->songs) <= 0){
					$r = Color::RED . "[PlayNBS] " . ($ik ? "당신은 음악이 하나도 없습니다." : "You don't have any song");
				}else{
			...
			case "shuffle":
			case "sh":
				if(!$sender->hasPermission("playnoteblocksong1.cmd1.shuffle1")){
					$r = new Translation(Color::RED . "%commands.generic.permission");
				}elseif(count($this->songs) <= 0){
					$r = Color::RED . "[PlayNBS] " . ($ik ? "당신은 음악이 하나도 없습니다." : "You don't have any song");
				}else{
			...
			case "list":
			case "l":
				if(!$sender->hasPermission("playnoteblocksong1.cmd1.list1")){
					$r = new Translation(Color::RED . "%commands.generic.permission");
				}elseif(count($this->songs) <= 0){
					$r = Color::RED . "[PlayNBS] " . ($ik ? "당신은 음악이 하나도 없습니다." : "You don't have any song");
				}else{
			...
			case "reload":
			case "r":
				if(!$sender->hasPermission("playnoteblocksong1.cmd1.reload1")){
					$r = new Translation(Color::RED . "%commands.generic.permission");
				}else{
					$this->loadSong();
	...
	public function sendSound($pitch, $type = NoteBlock::PIANO_OR_HARP){
		...
		foreach($this->getServer()->getOnlinePlayers() as $p) {
			if($p->getLevel()->getName() == "nt"){
				NoteBlock::runNoteBlockSound(new Position($p->x, $p->y + 1, $p->z, $p->level), $pitch, $type, $p);
