...
use pocketmine\event\entity\{EntityDamageByEntityEvent, EntityDamageEvent, EntityExplodeEvent, ProjectileLaunchEvent};
...
class EventListener implements Listener {
	...
	public function onHurt(EntityDamageEvent $event){
		...
		if($ent instanceof Player && $event instanceof EntityDamageByEntityEvent){
			...
			if(($reg = $this->plugin->getRegionByPlayer($ent)) !== ""){
				if(!$reg->getFlag("pvp") && $dmg instanceof Player){
					$dmg->sendMessage(TF::RED . 'You cannot hurt players of this region.');
					$event->setCancelled();
				}
			}
		}
