...
class BB extends PluginBase implements Listener {
        ...
        public $prefix = TE::GRAY . "§7«" . TE::GREEN. TE::BOLD . "Build" . TE::LIGHT_PURPLE."Battle". TE::RESET . TE::GRAY . "» ";
	public $mode = 0;
	public $arenas = array();
	public $currentLevel = "";
        public $op = array();
	...
	{
		$this->getLogger()->notice(TE::AQUA . "BuildBattle by atom91x");
		...
		foreach($this->arenas as $lev)
		{
			$this->getServer()->loadLevel($lev);
                ...
                $temas = array("Gaina","Pisica","Masina","Dragon","Internet","Vrajitor","Caine","Telefon","Tractor","Barca","Cascada","Tigru","Avion","Porc","Carte","Padure");
            ...
            if(in_array($level,$this->arenas))
            {
                ...
                if($time<=170)
                {
                    if($config->get("actual".$level)!=$player->getName())
                    {
                        ...
                        {
                            $player->sendTip(TE::BOLD.TE::GOLD."LEGENDAR");
                        }
                    }
                    else
                    {
                        $player->sendTip(TE::BOLD.TE::RED."Nu iti poti vota propriul plot");
                    }
                }
            }
        }
	...
	public function onInteract(PlayerInteractEvent $event)
	{
		...
		if($tile instanceof Sign) 
		{
			...
			{
				$tile->setText(TE::AQUA . "§7[ §bJoin §7]",TE::GREEN  . "§e0 / 16","§f" . $this->currentLevel,$this->prefix);
				$this->refreshArenas();
				$this->currentLevel = "";
				$this->mode = 0;
				...
				if($text[3] == $this->prefix)
				{
					if($text[0]==TE::AQUA . "§7[ §bJoin §7]")
					{
						$config = new Config($this->getDataFolder() . "/config.yml", Config::YAML);
                                                ...
                                                with:
                                                $slots->save();
                                                $player->sendMessage($this->prefix . "§eAi intrat intr-un meci de §bBuildBattle");
                                                foreach($level->getPlayers() as $playersinarena)
                                                {
                                                $playersinarena->sendMessage($player->getNameTag() .TE::AQUA. " §ejoined the game");
                                                }
					...
					{
						$player->sendMessage($this->prefix . "§cNu poti intra!");
					}
				}
			}
		}
					...
					else
					{
						...
						if($config->get($arena . "start")==1)
						{
							if($timeToStart>0)
							{
								...
								{
									$pl->sendTip(TE::WHITE."§bMeci-ul§e incepe in ".TE::GREEN . $timeToStart . TE::RESET);
                                                                        if($timeToStart<=5)
                                                                        {
							...
							else
							{
                                                                ...
                                                                if($time>170)
                                                                {
                                                                    foreach($playersArena as $pla)
                                                                    {
                                                                        $pla->sendTip(TE::BOLD.TE::AQUA."Theme: " .TE::GREEN. $tema .TE::AQUA. "  §etimp ramas: ".TE::YELLOW.$minutes.TE::DARK_PURPLE.":".TE::YELLOW.$seconds. TE::RESET);
                                                                    }
                                                                }
								$time--;
								if($time == 469)
								...
								if($time>170)
									...
									if(is_int($minutes) && $minutes>0)
									{
										foreach($playersArena as $pl)
										{
											$pl->sendMessage($this->prefix .TE::YELLOW. $minutes . " " .TE::GREEN."§eminute ramase");
										}
									}
									elseif($time2 == 30 || $time2 == 15 || $time2 == 10 || $time2 ==5 || $time2 ==4 || $time2 ==3 || $time2 ==2 || $time2 ==1)
									{
										foreach($playersArena as $pl)
										{
											$pl->sendMessage($this->prefix .TE::AQUA. $time2 . " " .TE::GREEN. "§esecunde ramase");
                                                                                        $levelArena->addSound(new PopSound($pl));
										}
									}
								}
                                                                else
                                                                {
                                                                    ...
                                                                    if($time==10)
                                                                    {
                                                                        ...
                                                                        foreach($playersArena as $pl) {
                                                                            ...
                                                                            $pl->sendMessage($this->prefix .TE::GREEN. "Scorul tau: ".TE::YELLOW.$tupos."°: ".TE::GREEN.$puntaje);
                                                                            $pl->teleport($spawn,0,0);
                                                                        }
                                                                    }
                                                                    if($time<=0)
						...
						else
						{
                                                    foreach($playersArena as $pl)
                                                    {
                                                            $pl->sendTip(TE::YELLOW . "Trebuie mai multi playeri" .TE::RESET);
                                                    }
                                                    $config->set($arena . "PlayTime", 470);
                                                    $config->set($arena . "StartTime", 30);
						}
