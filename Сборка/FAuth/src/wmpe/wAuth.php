...
public function onTap(	PlayerInteractEvent $e){
		...
		public function onPlayerJoin(PlayerJoinEvent $event) {
			...
			if(isset($user["nickname"])) {
				...
				if($ip == $user["ipLast"]) {
					...
					$player->sendMessage("§8(§bMystery§cBox§8)§f Вы уже вводили свой пароль! §aПриятной игры!");
				} else
					$player->sendMessage("§8(§bMystery§cBox§8)§f Введите §bсвой пароль §fв чат, который вводили при регистрации.");
			} else $player->sendMessage("§8(§bMystery§cBox§8)§f Придумайте и введите §bсвой пароль §fв чат, для регистрации.");
			$sql->finalize();
		}
		...
		public function onPlayerCommandPreprocess(PlayerCommandPreprocessEvent $event) {
			$login = "§8(§bMystery§cBox§8)§f Введите §bсвой пароль §fв чат, который вводили при регистрации.";
			$player = $event->getPlayer();
						...
						if($sql instanceof \SQLite3Result) {
							...
							if(!empty($user["nickname"])) {
								if($pass == $user["password"]) {
									...
									$player->sendMessage("§8(§bMystery§cBox§8)§f Вы успешно §aавторизировались §fна сервере. Приятной игры!");
								} else $player->sendMessage("§8(§bMystery§cBox§8)§f Вы ввели §cневерный пароль.");
							} else {
								if(!isset($this->reg[$name])) {
									$this->reg[$name] = $pass;
									$player->sendMessage("§8(§bMystery§cBox§8)§f Введите §c<свой пароль> §fв чат повторно!");
								} else {
									if($pass == $this->reg[$name]) {
										...
										$player->sendMessage("§8(§bMystery§cBox§8)§f Вы успешно §aзарегистрировались, §fне забудьте свой пароль!");
									} else {
										unset($this->reg[$name]);
										$player->sendMessage("§8(§bMystery§cBox§8)§f Вы ввели §cневерный пароль.");
									}
								}
							}
						}
