...
class AddCreditsCommand extends Command implements PluginIdentifiableCommand {
	...
	public function __construct(PrestigeSocietyCore $c){
		parent::__construct('addcredits', 'Add credits to player', 'Usage: /addcredits <player> <amount>', ['addcredits', 'givecredits']);
		$this->c = $c;
	}
	...
	public function execute(CommandSender $sender, string $commandLabel, array $args){
		...
		if($sender->hasPermission('credits.command.addcredits') || $sender->hasPermission('credits.command.all')){
			$player = $args[0];
			if(exc::checkIsNumber($credits = $args[1])){
				...
				$this->c->PrestigeSocietyCredits->addCredits($player, $credits);
				$message = $this->c->getMessage('credits', 'added_credits');
