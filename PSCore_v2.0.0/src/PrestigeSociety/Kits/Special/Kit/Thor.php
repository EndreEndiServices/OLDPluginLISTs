...
class Thor extends Kit {
	...
	public function __construct(Item $specialItem, array $items = [], int $coolDown, int $deactivate = -1){
		...
		DefaultPermissions::registerPermission(new Permission("kit.thor", "", Permission::DEFAULT_OP));
	}
		...
		if(($player instanceof Player) and ($item instanceof Item) and ($block instanceof Block)){
			...
			if(strtolower($data->getString("kit_name")) === "thor"){
				if($this->checkCoolDown($player)){
					...
					$player->level->broadcastLevelSoundEvent($p, LevelSoundEventPacket::SOUND_THUNDER, 93);
					...
					$player->getScheduler()->scheduleRepeatingTask(new ThorTask($this->getSpecialKitsInstance()->getCore(), $p->level, $blocks, $this->deactivate), 20);
...
class ThorTask extends PluginTask {
	...
	public function __construct(PrestigeSocietyCore $core, Level $level, array $blocks, int $seconds){
