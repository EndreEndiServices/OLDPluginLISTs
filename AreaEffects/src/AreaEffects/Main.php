...
class Main extends PluginBase implements Listener{
	...
	public function onLoad(){
		$this->getLogger()->info("§bArea§fEffects §bЗагружен!");
	}
	...
	public function onEnable(){
		...
		$this->getLogger()->info("§bArea§fEffects §eВключен!");
	}
				...
				case "pos1":
					{
						if($sender instanceof Player){
							...
							$sender->sendMessage("[§bArea§fEffects] §bПозиция 1 set as x:" . $pos1x . " y:" . $pos1y . " z:" . $pos1z);
							return true;
						}
						break;
					}
				...
				case "pos2":
					{
						if($sender instanceof Player){
							...
							$sender->sendMessage("[§bArea§fEffects] §bПозиция 2 set as x:" . $pos2x . " y:" . $pos2y . " z:" . $pos2z);
							return true;
						}
						break;
					}
							...
							if(isset($args[1], $args[2])){
								if(isset($this->pos1, $this->pos2)){
									if(($id = $this->isEffect($args[2])) !== null){
										...
										$this->saveConfig();
										$sender->sendMessage("[§bArea§fEffects] §fАрена §bДобавлена");
										return true;
									}
									else{
										$sender->sendMessage("[§bArea§fEffects] §cНеверный идентификатор эффекта или имя ");
										return false;
									}
								}
							}
							else{
								$sender->sendMessage("[§bArea§fEffects] §cНедостающие аргументы");
							}
						}
						else{
							$sender->sendMessage("[§bArea§fEffects] §cЭта комманда работает в игре!");
						}
						break;
					}
				...
				case "remove":
					{
						if(isset($args[1])){
							...
							if(!$this->getConfig()->exists($args[1])){
								$sender->sendMessage("[§bArea§fEffects] §eАрена удалена");
								return true;
							}
							else{
								$sender->sendMessage("[AreaEffects] §cОшибка удаления области");
								return true;
							}
						}
						else{
							$sender->sendMessage("[§bArea§fEffects] §cИспользуйте /ae remove <name>");
						}
						break;
					}
	...
	public function isInArea(Player $player, $areaname){
		...
		if(($player->getLevel()->getName() == $area['level']) and ($player->getFloorX() <= max($area['pos1']['x'], $area['pos2']['x'])) and ($player->getFloorX() >= min($area['pos1']['x'], $area['pos2']['x'])) and ($player->getFloorY() <= max($area['pos1']['y'], $area['pos2']['y'])) and ($player->getFloorY() >= min($area['pos1']['y'], $area['pos2']['y'])) and ($player->getFloorZ() <= max($area['pos1']['z'], $area['pos2']['z'])) and ($player->getFloorZ() >= min($area['pos1']['z'], $area['pos2']['z']))){
			if(is_null($this->isEffect($area['effect']['id']))){
				...
				$message = TextFormat::YELLOW . "[§bArea§fEffects] §cНайден неверный эффект,арена удалена " . TextFormat::GRAY . $areaname;
