...
class Prefix extends Command
{
    public function __construct(PermsMain $plugin, $name, $description, $perm){
        $this->plugin = $plugin;
		$this->perm = $perm;
        parent::__construct($name, $description, null, array("setprefix"));
    }
	...
	public function execute(CommandSender $sender, $label, array $args){
		...
		if(!isset($args[0]))
			return $sender->sendMessage("§6[§ePerms§6] §cИспользуйте: /prefix <новый_префикс>");
		
		if(isset($args[1]))
			return $sender->sendMessage("§6[§ePerms§6] §cПрефикс не должен содержать пробелов!");
		
		if(!preg_match("#^[aA-zZ0-9\§_]+$#", $args[0]))
			return $sender->sendMessage("§6[§ePerms§6] §cПрефикс должен быть только из цифр, букв и знаков цвета!");
		
		if(strlen($args[0]) > 20 or strlen($args[0]) < 3)
			return $sender->sendMessage("§6[§ePerms§6] §cПрефикс не должен превышать 20 и быть меньше 3 символов.");
		...
		$this->plugin->setPlayerInfo($sender->getName(), array("group" => $info["group"], "nick" => $info["nick"], "prefix" => $prefix));
		...
		$sender->sendMessage("§6[§ePerms§6] §eПрефикс успешно обновлен");
