...
class Bedwars extends PluginBase implements Listener {
    ...
    public $prefix = TextFormat::GRAY."[".TextFormat::RED."§4Bed§fWars" .TextFormat::GRAY."]".TextFormat::WHITE." ";
    ...
    public function onEnable(){
        ...
        $this->getLogger()->info($this->prefix.TextFormat::GREEN."Плагин BedWars был запущен");
    ...
    public function resetArena($arena, $mapreset = false){
        ...
        $this->getLogger()->info(TextFormat::GREEN."На арене ".TextFormat::AQUA.$arena.TextFormat::GREEN." игра успешно завершена");
    }
    ...
    public function sendIngameScoreboard(Player $p, $arena){
        $config = new Config($this->getDataFolder()."Arenas/".$arena.".yml", Config::YAML);
        $popup = TextFormat::GRAY." [".TextFormat::GOLD."Teams".TextFormat::GRAY."]\n";
        $teams = $this->getTeams($arena);
                ...
                if($this->getArenaStatus($arena) == "Lobby") {
                    if($team != $this->getTeam($player->getNameTag())){
                        if (in_array($team, $this->getAvailableTeams($arena))) {
                            ...
                            $player->sendMessage($this->prefix . "Теперь ты в команде " . TextFormat::GOLD . $team);
                            $player->getInventory()->removeItem($item);
                            $player->getInventory()->addItem($item);
                        } else {
                            $player->sendMessage($this->prefix . "Команда " . TextFormat::GOLD . $team . TextFormat::WHITE . " уже заполнена!");
                            $player->getInventory()->removeItem($item);
                            $player->getInventory()->addItem($item);
                        }
                    } else {
                        $player->sendMessage($this->prefix . "Вы уже в команде " . TextFormat::GOLD . $team);
                        $player->getInventory()->removeItem($item);
                        $player->getInventory()->addItem($item);
                    }
                }
                    ...
                    foreach ($players as $pn) {
                        ...
                        if ($p != null) {
                            if ($this->getTeam($p->getNameTag()) == $this->getTeam($player->getNameTag())) {
                                //teamchat
                                $p->sendMessage(TextFormat::GRAY . "[" . $this->getTeamColor($this->getTeam($player->getNameTag())) . "Чат_Команды" . TextFormat::GRAY . "] " . $player->getNameTag() . TextFormat::GRAY . " >> " . TextFormat::WHITE . $msg);
                            }
                        }
                    }
                }
    ...
    public function onRespawn(PlayerRespawnEvent $event){
        ...
        if($this->inArena($player)){
            ...
            if($config->getNested("Bed.".$team.".Alive") == true){
                ...
                $player->sendMessage($this->prefix.TextFormat::RED."Ваша кровать не разрушена, Вы, можете вожете возводиться!");
            ...
            if($this->inArena($player)){
                ...
                if(!in_array($event->getItem()->getId(), $this->pickup)) {
                    if ($event->getItem()->getItem()->getId() == Item::BRICK) {
                        ...
                        $player->sendTip(TextFormat::GOLD . "+" . TextFormat::GREEN . "1 очко!");
                    }
                    ...
                    if ($event->getItem()->getItem()->getId() == Item::IRON_INGOT) {
                        ...
                        $player->sendTip(TextFormat::GOLD . "+" . TextFormat::GREEN . "10 очков!");
                    }
                    ...
                    if ($event->getItem()->getItem()->getId() == Item::GOLD_INGOT) {
                        ...
                        $player->sendTip(TextFormat::GOLD . "+" . TextFormat::GREEN . "20 очков!");
                    }
                }
            }
        }
                ...
                if ($cause instanceof EntityDamageByEntityEvent) {
                    ...
                    if ($killer instanceof Player) {
                        foreach ($players as $pn) {
                            $p = $this->getServer()->getPlayerExact($pn);
                            if($p != null) {
                                $p->sendMessage($this->prefix . $killer->getNameTag() . TextFormat::GRAY. " убил " . $player->getNameTag() . TextFormat::GRAY . "");
                            }
                        }
                    } else {
                        foreach ($players as $pn) {
                            $p = $this->getServer()->getPlayerExact($pn);
                            if($p != null) {
                                $p->sendMessage($this->prefix . $player->getNameTag() . TextFormat::GRAY . " умер!");
                            }
                        }
                    }
                } else {
                    ...
                    foreach ($players as $pn) {
                        ...
                        if($p != null) {
                            ...
                            if($this->lasthit[$player->getName()] != "no"){
                                ...
                                if($p2 != null){
                                    $p->sendMessage($this->prefix . $p2->getNameTag() . TextFormat::WHITE. " убил " . $player->getNameTag() . TextFormat::WHITE . "");
                                    $this->lasthit[$player->getName()] = "no";
                                } else {
                                    $p->sendMessage($this->prefix . $player->getNameTag() . TextFormat::GRAY . " умер!");
                                }
            ...
            if($config->get("Status") == "Lobby"){
                ...
                if($block->getId() == Block::WOOL){
                    ...
                    if($team != $this->getTeam($player->getNameTag())){
                        if (in_array($team, $this->getAvailableTeams($arena))) {
                            $player->setNameTag($this->getTeamColor($team) . $name);
                            $player->sendMessage($this->prefix . "Теперь Вы в команде " . TextFormat::GOLD . $team);
            ...
            if($config->get("Status") != "Lobby"){
                ...
                if($block->getId() == Block::BED_BLOCK) {
                    ...
                    if ($team != $this->getTeam($player->getNameTag())) {
                        ...
                        $player->sendMessage($this->prefix . "Кровать команды " . $team . " была разрушена!");

                        foreach ($this->getPlayers($arena) as $pn) {
                            $p = $this->getServer()->getPlayerExact($pn);
                            if ($p != null) {
                                if ($team == $this->getTeam($p->getNameTag())) {
                                    $p->sendMessage($this->prefix . TextFormat::RED . "Кровать вашей команды была разрушена");
                                } else {
                                    $p->sendMessage($this->prefix . "Кровать команды " . TextFormat::GOLD . $team . TextFormat::WHITE . " была разрушена");
                                }

                            }
                        }
                    } else {
                        $player->sendMessage($this->prefix . "Вы не можете разрушить свою кровать!");
                        $event->setCancelled();
                    }
                }
    ...
    public function onInteract(PlayerInteractEvent $event){
        ...
        if($this->registerBed == true && $this->registerBedWHO == $name){
            ...
            $player->sendMessage(TextFormat::GREEN . "Вы успешно создали кровать команды " . TextFormat::AQUA . $team . TextFormat::GREEN . " на арене " . TextFormat::AQUA . $arena . TextFormat::GREEN . " !");
            $player->sendMessage(TextFormat::GREEN . "Используйте -> /bw help");
        }
        ...
        if($tile instanceof Sign){
            ...
            if($this->registerSign == true && $this->registerSignWHO == $name){
                ...
                $tile->setText($this->prefix, $arena." ".$teams."x".$ppt, TextFormat::GREEN."Загрузка...", TextFormat::YELLOW."0 / ".$maxplayers);
                $this->registerSign = false;

                $player->sendMessage(TextFormat::GREEN . "Вы успешно вошли на арену  " . TextFormat::AQUA . $arena . TextFormat::GREEN . " !");
                $player->sendMessage(TextFormat::GREEN . "Используйте -> /bw help");
            }
            elseif($text[0] == $this->prefix){

                if($text[2] == TextFormat::GREEN."Войти"){
                    ...
                    if($status == "Lobby"){
                        if($players < $maxplayers) {
                            ...
                            $player->sendMessage($this->prefix . TextFormat::RED . "Вы не можете войти в этот матч");
                        }
                    } else {
                        $player->sendMessage($this->prefix.TextFormat::RED."Вы не можете войти в этот матч!");
                    }
        ...
        if($cmd->getName() == "Bedwars" && $sender->isOP()){
            if(!empty($args[0])){
                if(strtolower($args[0]) == "help" && $sender->isOP()){
                    ...
                    $sender->sendMessage(TextFormat::GRAY."-> ".TextFormat::DARK_AQUA."/bw help ".TextFormat::GRAY."[".TextFormat::RED."Показывает все команды BW".TextFormat::GRAY."]");
                    $sender->sendMessage(TextFormat::GRAY."-> ".TextFormat::DARK_AQUA."/bw regsign <Арена> ".TextFormat::GRAY."[".TextFormat::RED."Зарегистрировать табличку для входа на арену".TextFormat::GRAY."]");
                    $sender->sendMessage(TextFormat::GRAY."-> ".TextFormat::DARK_AQUA."/bw savemaps <Арена> ".TextFormat::GRAY."[".TextFormat::RED."Сохранение карты Арены".TextFormat::GRAY."]");
                    $sender->sendMessage(TextFormat::GRAY."-> ".TextFormat::DARK_AQUA."/bw addarena <НазваниеАрены> <Команда> <Игроков> ".TextFormat::GRAY."[".TextFormat::RED."Создание арены".TextFormat::GRAY."]");
                    $sender->sendMessage(TextFormat::GRAY."-> ".TextFormat::DARK_AQUA."/bw setlobby <Арена>".TextFormat::GRAY."[".TextFormat::RED."Лобби для выбора команды".TextFormat::GRAY."]");
                    $sender->sendMessage(TextFormat::GRAY."-> ".TextFormat::DARK_AQUA."/bw setspawn <Арена> <Команда>".TextFormat::GRAY."[".TextFormat::RED."Регистрация точки спавна команд".TextFormat::GRAY."]");
                    $sender->sendMessage(TextFormat::GRAY."-> ".TextFormat::DARK_AQUA."/bw setbed <Арена> <Команда>".TextFormat::GRAY."[".TextFormat::RED."Регистрация кровати для команды".TextFormat::GRAY."]");
                    $sender->sendMessage(TextFormat::GRAY."===============");
                }
                elseif(strtolower($args[0]) == "regsign" && $sender->isOP()){
                    if(!empty($args[1])) {
                        ...
                        if($this->arenaExists($arena)) {
                            ...
                            $sender->sendMessage(TextFormat::GREEN . "Теперь нажмите на табличку!");
                        } else {
                            $sender->sendMessage(TextFormat::RED."Арена не существует!");
                        }
                    } else {
                        $sender->sendMessage(TextFormat::RED."/bw regsign <НазваниеАрены>");
                    }
                }
                elseif(strtolower($args[0]) == "savemaps" && $sender->isOP()){
                    if(!empty($args[1])) {
                        ...
                        if($this->arenaExists($arena)) {
                            $this->saveMaps($arena);
                            $sender->sendMessage(TextFormat::GREEN . "Вы успешно сохранили карту для арены " . TextFormat::AQUA . $arena . TextFormat::GREEN . " !");
                        } else {
                            $sender->sendMessage(TextFormat::RED."Арена не существует");
                        }
                    } else {
                        $sender->sendMessage(TextFormat::RED."/bw savemaps <НазваниеАрены>");
                    }
                }
                elseif(strtolower($args[0]) == "addarena" && $sender->isOP()){
                    if(!empty($args[1]) && !empty($args[2]) && !empty($args[3])) {
                        ...
                        if($teams <= 8){
                            ...
                            $this->arena = $arena;
                            $sender->sendMessage(TextFormat::GREEN . "Арена " . TextFormat::AQUA . $arena . TextFormat::GREEN . " успешно создана!");
                            $sender->sendMessage(TextFormat::GREEN . "Используйте -> /bw help");
                        } else {
                            $sender->sendMessage(TextFormat::RED."Максимальное количество команд 8");
                        }
                    } else {
                        $sender->sendMessage(TextFormat::RED."/bw addarena <ArenaName> <Команд> <Игроков>");
                    }
                }
                elseif(strtolower($args[0]) == "setlobby" && $sender->isOP()){
                    if(!empty($args[1])) {
                        ...
                        if($this->arenaExists($arena)) {
                            ...
                            $sender->sendMessage(TextFormat::GREEN . "Лобби было успешно добавлено на арене " . TextFormat::AQUA . $arena . TextFormat::GREEN . " !");
                            $sender->sendMessage(TextFormat::GREEN . "Используйте -> /bw help");
                        ...
                        $sender->sendMessage(TextFormat::RED."/bw setlobby <НазваниеАрены>");
                    }
                }
                    ...
                    if(!empty($args[1]) && !empty($args[2])) {
                        ...
                        if($this->arenaExists($arena)) {
                            if (in_array($team, $this->getAllTeams())) {
                                ...
                                $sender->sendMessage(TextFormat::GREEN . "Теперь нажмите на кровать! Нажимайте на нижнюю часть кровати, так как это может призвисти к ошибке!");

                                $this->resetArena($arena);
                            } else {
                                $alleteams = implode(" ", $this->getAllTeams());

                                $sender->sendMessage(TextFormat::RED . "Команда " . TextFormat::GOLD . $team . TextFormat::RED . " Не существует!");
                                $sender->sendMessage(TextFormat::RED . "Команды: " . $alleteams);
                            }
                        } else {
                            $sender->sendMessage(TextFormat::RED."Арена не существует!");
                        }
                    } else {
                        $sender->sendMessage(TextFormat::RED."/bw setbed <НазваниеАрены> <Команда>");
                    }
                }
                    ...
                    if(!empty($args[1]) && !empty($args[2])) {
                        ...
                        if($this->arenaExists($arena)) {
                            if (in_array($team, $this->getAllTeams())) {
                                ...
                                $sender->sendMessage(TextFormat::GREEN . "Спавн команды ".TextFormat::AQUA . $team . TextFormat::GREEN." на арене" . TextFormat::AQUA . $arena . TextFormat::GREEN . " был успешно добавлен");
                                $sender->sendMessage(TextFormat::GREEN . "Используйте -> /bw help");
                        ...
                        $sender->sendMessage(TextFormat::RED."/bw setspawn <НазваниеАрены> <Команда>");
                    }
                }
        ...
        foreach ($tiles as $t) {
            if ($t instanceof Sign) {
                ...
                if ($text[0] == $this->prefix) {
                    ...
                    $maxplayers = $teams * $ppt;
                    $ingame = TextFormat::GREEN."Войти";

                    if ($status != "Lobby") {
                        $ingame = TextFormat::RED . "Идет Игра";
                    }
                    if (count($players) >= $maxplayers) {
                        $ingame = TextFormat::RED . "Заполнена";
                    }
                    if ($status == "Ende") {
                        $ingame = TextFormat::RED . "Перезагрузка";
                    }
                ...
                if($status == "Lobby"){
                    ...
                    if(count($players) < $minplayers){
                        ...
                        foreach($players as $pn){
                            ...
                            if($p != null) {
                                $p->sendPopup(TextFormat::RED . "Ждем ещё  ".TextFormat::GOLD.$minplayers.TextFormat::RED." участников");
                            } else {
                        ...
                        if((Time() % 20) == 0){
                            foreach($players as $pn){
                                $p = $this->plugin->getServer()->getPlayerExact($pn);
                                if($p != null) {
                                    $p->sendMessage(TextFormat::GOLD . $minplayers . TextFormat::RED ." игроков не хватает для начала игры");
                                } else {
                                    $this->plugin->removePlayerFromArena($arena, $pn);
                                }
                            }
                        ...
                        if($lobbytimer == 60 ||
                            ...
                            foreach($players as $pn){
                                $p = $this->plugin->getServer()->getPlayerExact($pn);
                                if($p != null){
                                    $p->sendMessage($this->prefix."Игра начнется через ".$lobbytimer." секунд!");
                                }
                            }
                        }
                        if($lobbytimer >= 1 && $lobbytimer <= 5){
                            foreach($players as $pn){
                                $p = $this->plugin->getServer()->getPlayerExact($pn);
                                if($p != null){
                                    $p->sendPopup(TextFormat::YELLOW."Старт через ".TextFormat::RED.$lobbytimer);
                                } else {
                                    $this->plugin->removePlayerFromArena($arena, $pn);
                                }
                            }
                ...
                elseif ($status == "Ingame"){
                    if(count($aliveTeams) <= 1){
                        if(count($aliveTeams) == 1){
                            ...
                            $this->plugin->getServer()->broadcastMessage($this->prefix."Команда ".TextFormat::GOLD.$winnerteam.TextFormat::WHITE." §aВЫИГРАЛА на арене ".TextFormat::GOLD.$arena.TextFormat::WHITE." !");
                        }
                        ...
                        if($gametimer==900||$gametimer==600|| $gametimer==300|| $gametimer==240 || $gametimer==180){
                            foreach($players as $pn){
                                $p = $this->plugin->getServer()->getPlayerExact($pn);
                                if($p != null){
                                    $p->sendMessage($this->plugin->prefix.$gametimer/60 . " минут осталось");
                                } else {
                                    $this->plugin->removePlayerFromArena($arena, $pn);
                                }
                            }
                        ...
                        elseif($gametimer == 2||$gametimer == 3|| $gametimer==4||$gametimer==5||$gametimer==15||$gametimer==30||$gametimer==60){
                            foreach($players as $pn){
                                $p = $this->plugin->getServer()->getPlayerExact($pn);
                                if($p != null){
                                    $p->sendMessage($this->plugin->prefix.$gametimer . " секунд осталось");
                                } else {
                                    $this->plugin->removePlayerFromArena($arena, $pn);
                                }
                            }
                        ...
                        elseif($gametimer == 1){
                            foreach($players as $pn){
                                $p = $this->plugin->getServer()->getPlayerExact($pn);
                                if($p != null){
                                    $p->sendMessage($this->plugin->prefix."1 секунда осталась");
                                } else {
                                    $this->plugin->removePlayerFromArena($arena, $pn);
                                }
                            }
                        ...
                        elseif($gametimer==0){
                            foreach($players as $pn){
                                $p = $this->plugin->getServer()->getPlayerExact($pn);
                                if($p != null){
                                    $p->sendMessage($this->plugin->prefix."Дед-Матч стартовал!");

                                    $p->sendMessage($this->plugin->prefix."Победителей не было найдено!");
                    ...
                    if($endtimer >= 0){
                        ...
                        if($endtimer == 15 ||
                            ...
                            foreach($players as $pn){
                                $p = $this->plugin->getServer()->getPlayerExact($pn);
                                if($p != null){
                                    $p->sendMessage($this->plugin->prefix."Перезагрузка арены через ".$endtimer." секунд!");
                                } else {
                                    $this->plugin->removePlayerFromArena($arena, $pn);
                                }
                            }
